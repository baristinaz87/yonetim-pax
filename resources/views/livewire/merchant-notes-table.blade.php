    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500">
        ...
        @foreach($data as $note)
            <tr class="bg-white border-b border-gray-200 hover:bg-gray-50">
                ...
                <td class="px-6 py-4">
                    <a href="#" data-modal-target="popup-modal-{{ $note["id"] }}" data-modal-toggle="popup-modal-{{ $note["id"] }}" onclick="event.preventDefault();" class="font-medium text-red-600 hover:underline">
                        Sil
                    </a>
                    <div id="popup-modal-{{ $note["id"] }}" tabindex="-1" class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                        <div class="relative p-4 w-full max-w-md max-h-full">
                            <div class="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
                                <button type="button" class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" data-modal-hide="popup-modal-{{ $note["id"] }}">
                                ...
                                <div class="p-4 md:p-5 text-center">
                                    ...
                                    <button wire:click="removeNote({{ $note["id"] }})" data-modal-hide="popup-modal-{{ $note["id"] }}" type="button" class="text-white bg-red-600 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 font-medium rounded-lg text-sm inline-flex items-center px-5 py-2.5 text-center">
                                        Evet, sil
                                    </button>
                                    <button data-modal-hide="popup-modal-{{ $note["id"] }}" type="button" class="py-2.5 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100 dark:focus:ring-gray-700 dark:bg-gray-800 dark:text-gray-400 dark:border-gray-600 dark:hover:text-white dark:hover:bg-gray-700">Vazgeç</button>
                                </div>
                            </div>
                        </div>
                    </div>
