            ...
            <form wire:submit.prevent="save" class="space-y-5">
                <div class="grid gap-4 lg:grid-cols-4">
                    ...
                    <div>
                        ...
                        <select wire:model.live="form.event_type" class="block w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring focus:ring-blue-200">
                            @foreach($eventTypes as $handle => $eventName)
                                <option value="{{ $handle }}">{{ $eventName }} ({{ $handle }})</option>
                            @endforeach
                        </select>
                    ...
                    <tbody>
                        @forelse($flows as $flow)
                            <tr class="bg-white border-b hover:bg-gray-50">
                                ...
                                <td class="px-6 py-4">{{ $eventTypes[$flow->event_type] ?? $flow->event_type }}</td>
                                <td class="px-6 py-4">
