    ...
    <div class="mb-4 flex flex-wrap justify-between items-center gap-4">
        <div class="flex flex-wrap gap-4">
            <div class="relative">
                ...
                <input wire:model.live.debounce.300ms="search" type="text" class="block w-72 pl-10 pr-3 py-2 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500" placeholder="Uygulama ara...">
            </div>

            <select wire:model.live="partnerFilter" class="border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring focus:ring-blue-200">
                <option value="">Tüm Partner Hesapları</option>
                <option value="none">— Partner hesabı atanmamış —</option>
                @foreach($partners as $partner)
                    <option value="{{ $partner->id }}">{{ $partner->name }} ({{ $partner->org_id }})</option>
                @endforeach
            </select>
        ...
        <table class="w-full text-sm text-left text-gray-500">
            <thead class="text-xs text-gray-700 uppercase bg-gray-50">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3">Partner Hesabı</th>
                    <th scope="col" class="px-6 py-3 text-center">Mağaza</th>
                    <th scope="col" class="px-6 py-3 text-center">Aktif Kurulum</th>
                    <th scope="col" class="px-6 py-3">Son Sync</th>
                    ...
                    <tr class="bg-white border-b hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4">
                            @if($app->partnerAccount)
                                <span class="text-blue-700">{{ $app->partnerAccount->name }}</span>
                                <span class="text-gray-400 text-xs font-mono">({{ $app->partnerAccount->org_id }})</span>
                            @else
                                <span class="bg-yellow-100 text-yellow-800 text-xs font-medium px-2 py-0.5 rounded">Atanmamış</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-center">{{ $app->stores_count }}</td>
                        <td class="px-6 py-4 text-center">{{ $app->active_stores_count }}</td>
                        <td class="px-6 py-4 text-xs">
                            {{ $app->last_synced_at?->diffForHumans() ?? '—' }}
                        </td>
                        <td class="px-6 py-4">
                            @if($app->active)
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="inline-flex gap-2">
                                ...
                                <button wire:click="toggleActive({{ $app->id }})" class="text-yellow-600 hover:text-yellow-900" title="{{ $app->active ? 'Pasif Yap' : 'Aktif Yap' }}">
                                    @if($app->active)
                                ...
                                <button wire:click="deleteApp({{ $app->id }})" wire:confirm="Bu uygulamayı ve tüm ilişkili event/kurulum kayıtlarını silmek istediğinizden emin misiniz?" class="text-red-600 hover:text-red-900" title="Sil">
                ...
                @empty
                    <tr>
                        <td colspan="8" class="px-6 py-8 text-center text-gray-500">
                            Henüz uygulama yok.
                            <a href="{{ route('shopify.apps.create') }}" class="text-blue-600 hover:underline">İlk uygulamayı ekleyin →</a>
                        </td>
                    </tr>
                @endforelse
            </tbody>
