    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                            ...
                            @foreach($status as $key => $value)
                                <li>
                                    <div class="flex items-center p-2 rounded-sm hover:bg-gray-100">
                                        <input wire:click="$set('selectedStatus', '{{$key}}')" id="choice-{{$key}}" type="radio" value="{{$key}}" name="filter-radio" class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 focus:ring-2">
                                        <label for="choice-{{$key}}" class="w-full ms-2 text-sm font-medium text-gray-900 rounded-sm">{{$value}}</label>
                                    </div>
                                </li>
                            @endforeach
                        </ul>
        ...
        @foreach($data as $customer)
            <tr class="bg-white border-b border-gray-200 hover:bg-gray-50">
                ...
                <td class="px-6 py-4">
                    <a href="{{ route('merchant-detail', ['id' => $customer['id']]) }}" class="font-medium text-blue-600 hover:underline">Düzenle</a>
                </td>
            </tr>
        @endforeach
        </tbody>
